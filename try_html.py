...
#         print("\n\n\nEnding ... \n",word_list[0], "Word List1 : ", print_dict(word_list[0])) 
#         print("\n",word_list[1],"Word List2 : ", print_dict(word_list[1]))
#     # return doc_dict
...
#     sec_word = user_input_unique_list[1]
#     sec_letter = sec_word[0]
#     mycol2 = db[sec_letter] # collection db
...
#     # cursor = mycol.find({"_id":first_word , 'doc_info.originalID':1, "doc_info.tagScore":1}) # _id, doc_info
...
#     # cursor = mycol.find({"_id":first_word}, {"doc_info": {'originalID': 1, 'tagScore': 1}}) # this is working !! 

#     # doc_info_query = {"doc_info": {'originalID': 1, 'tagScore': 1}}
#     # cursor = mycol.find({},{"_id":first_word, "doc_info.originalID":1, "doc_info.tagScore": 1}).sort("doc_info.tagScore", -1)

#     # cursor = mycol.find({"_id":first_word}, {"doc_info": {'originalID': 1, 'tagScore': 1}})
#     # cursor = mycol.find({"_id":first_word}, {"doc_info.originalID", "doc_info.tagScore"}).sort("doc_info.tagScore", pymongo.ASCENDING)


#     # cursor = mycol.find({},{"_id":1, "total":1}).sort("total", -1) # sorts by total
#     # cursor = mycol.find({},{"_id":1, "total":1}).sort("total", -1).limit(5) # sorts by total limits to 5
#     # cursor = mycol.find({"_id":first_word}, {"doc_info": {"originalID": 1, "tagScore": 1}}).sort("tagScore", -1)
#     # cursor = mycol.find({"_id":first_word}, {"doc_info": {'$elemMatch':{'originalID': 1, 'tagScore': 1}}})
#     # cursor = mycol.findone({"_id":first_word}, {"doc_info": {'originalID': 1, 'tagScore': 1}})
...
#     # # cursor = cursor.sort("tagScore", 1)
#     # # get tag score by DESCENDING
#     # cursor = mycol.aggregate([
#     #         {"$match": {"_id": first_word}}, # matches with the id = word
#     #         {"$unwind": "$doc_info"}, # it extracts the array ! gets the whole array
#     #         {"$project":{
#     #             "doc": "$doc_info.originalID",
#     #             "tagScore": "$doc_info.tagScore"
...
#     # cursor = mycol.aggregate([
#     #         {"$match": {"_id": first_word}}, # matches with the id = word 
#     #         {"$unwind": "$doc_info"}, # it extracts the array ! gets the whole array # unwinds an array
#     #         {"$project":{ # format for printing
#     #             "doc": "$doc_info.originalID", # doc = doc_info.originalID
#     #             "tagScore": "$doc_info.tagScore", # tagScore = doc_info.tagScore
#     #             "norm": "$doc_info.normalized" # norm = doc_info.normalized
#     #         }},
#     #         {"$sort": { "tagScore": pymongo.DESCENDING}}, 
#     #         {"$limit" : 30 }
#     #     ])
...
#     # 4 words = antony  brutus caesar calpurnia
#     # # look for docids that appears 4,3,2,1
#     # intersection docids in 4 words
#     #                        3      -> antony brutus caesar, brutus ceasar calpurnia  
#     #                        2 
#     # 1
#     # [

#     # ]
#     # which docis appears 4 times or 3 times 
...
#         print("doc: ", document['doc'])
#         print("tagScore: ", document['tagScore'])
#         print("norm: ", document['norm'])
#         print()

#         # print(type(document))
#         # print(doc) #pymongo print  type 
# #     # # print()

# document_weight_tag_limit30(db, ["computer"])
